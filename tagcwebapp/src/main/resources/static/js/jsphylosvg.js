    ...
    Smits.PhyloCanvas.NexmlParse.prototype = {
        ...
        return function(path, params){
            ...
            if(params) {
                ...
                if (params.attr) this.attr = params.attr;
                if (params.node) {
                    if (params.node.id) this.attr.id = params.node.id;
                    if (params.node.children.length > 1) this.attr.children = 1;
                    params.attr.class = "middleNode";
                }
            }
        }
    ...
    Smits.PhyloCanvas.Render.SVG.prototype = {
        ...
        draw : function(instruct){
           ...
           if(instruct.type == 'line'){
            ...
            } else if(instruct.type == 'node'){
               obj = this.svg.circle(instruct.x, instruct.y, instruct.radius).attr({
                   "stroke": 'black',
                   "fill": 'black',
                   "nodeId": 'nodeId'
               });
               obj.node.setAttribute('nodeid', instruct.nodeId);
